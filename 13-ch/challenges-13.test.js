...
const standardizePhoneNumbers = (arr) => {
  let newArray =[];
  arr.forEach (element=> {
    let newNumber = element.substring(1,4)+element.substring(6,9)+ element.substring(10);
...
const unenrollBrook = (arr) => {
  return arr.map(element => element.filter(subElement =>
    !subElement.includes('Brook')))
};
